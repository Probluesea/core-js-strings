...
function isPalindrome(str) {
  const inputStr = str.match(/\w/g).join('').toLowerCase();

  for (let i = 0; i < inputStr.length; i += 1) {
    const startChar = inputStr.at(i);
    const endChar = inputStr.at(-i - 1);
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    const charIndex = alphas.indexOf(char);

    if (charIndex >= 0) {
      const targetChar = alphas[charIndex + 13];
