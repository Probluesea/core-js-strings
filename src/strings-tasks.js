...
function removeFirstOccurrences(str, value) {
  ...
  if (occurrenceIndex >= 0) splitedString.splice(occurrenceIndex, value.length);
...
function removeLastOccurrences(str, value) {
  const splitedString = str.split('');
  const occurrenceIndex = str.lastIndexOf(value);
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < formatedStr.length; i += 1) {
    const startchar = formatedStr[i];
    const endChar = formatedStr[formatedStr.length - i - 1];
...
function findLongestWord(sentence) {
  ...
  const sortedSentence = splitedSentence.sort((a, b) => b.length - a.length);
  const longestWord = sortedSentence[0].word;
